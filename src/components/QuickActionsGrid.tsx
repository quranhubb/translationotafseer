...
export const QuickActionsGrid: React.FC<QuickActionsGridProps> = ({
  ...
  return (
    <div className="bg-[#FFFFFF] rounded-2xl p-4 shadow-xs border border-[#E5DFCE] mb-3.5 mx-3 mt-3">
        ...
        {actions.map((item) => {
          ...
          return (
            <button
              ...
              className="flex flex-col items-center justify-center text-center group cursor-pointer p-1.5 rounded-xl hover:bg-[#F7F3E8] active:scale-95 transition-all"
            >
              <div className="w-10 h-10 rounded-full flex items-center justify-center text-[#7D6B4B] bg-[#F7F3E8] border border-[#E5DFCE] group-hover:bg-[#7D6B4B] group-hover:text-white group-hover:border-[#7D6B4B] transition-all mb-1.5 shadow-2xs">
                <Icon className="w-4.5 h-4.5 transition-transform group-hover:scale-110" />
              </div>
              <span className="text-[11px] font-medium text-[#4A4031] leading-tight group-hover:text-[#7D6B4B] font-sans">
                {item.label}
              </span>
            </button>
          );
